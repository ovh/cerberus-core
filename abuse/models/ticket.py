...
from django.db.models import (DateTimeField, ForeignKey, Q,
...
class Ticket(CerberusModel):
    ...
    defendant = ForeignKey(
        'Defendant',
        null=True,
        related_name='ticketDefendant',
        on_delete=PROTECT
    )
    category = ForeignKey(
        'Category',
        null=False,
        related_name='ticketCategory',
        on_delete=PROTECT
    )
    ...
    status = TruncatedCharField(
        ...
        null=False, choices=TICKET_STATUS,
        default='Open'
    )
    ...
    @classmethod
    def create_ticket(cls, defendant, category, service,
                      priority='Normal', attach_new=True):
        """
            Create a new ticket
    ...
    def set_higher_priority(self):
        ...
        priorities = list(set(self.reportTicket.all().values_list(
            'provider__priority', flat=True)
        ))
        ...
        if self.defendant:  # Warning for new customer or "big" ticket
            ...
            if self.PRIORITY_LEVEL[priority] > self.PRIORITY_LEVEL['High']:
                ...
                if (defendant.details.creationDate >= date_limit or
                        self.reportTicket.count() > 50):
                    priority = 'High'
    ...
    def get_emailed_providers(self, email_only=True):
        ...
        provider_emails = Provider.filter(
            provider__ticket=self
        ).values_list(
            'email', flat=True
        ).distinct()

        logs = list(set(self.ticketHistory.filter(
    ...
    def get_reports_provider_email(self):
        ...
        return list(set(Provider.filter(
            provider__ticket=self
        ).values_list(
            'email', flat=True
        ).distinct()))
    ...
    def has_defendant_email_responses(self):
        ...
        for email in emails:
            if (email.category.lower() == 'defendant' and
                    email.sender.lower() in senders):
                return True
        return False
    ...
    def has_defendant_email_requests(self):
        ...
        for email in emails:
            if (email.category.lower() == 'defendant' and
                    email.recipient.lower() in recipients):
                return True
        return False
    ...
    def get_service_action_ipaddr(self):
        ...
        if not ips:
            raise NoIpaddrItems(
                'ticket {} has no ipaddr items'.format(self.id)
            )
        if len(ips) > 1:
    ...
    def verify_service_action_ipaddr(self, ip_addr):
        ...
        ticket_items = self.reportTicket.all().values_list(
        ...
        if ip_addr not in ips:
            raise ValueError(
                'Specified IP address not attached to ticket'
            )
    ...
    def unpause(self, user=None):
        ...
        """
        # Delay jobs
        delay = timedelta(seconds=self.pauseDuration) - (datetime.now() - self.pauseStart)
        enqueue('ticket.delay_jobs', ticket=self.id, delay=delay, back=True)

        if self.previousStatus == 'WaitingAnswer' and self.snoozeDuration and self.snoozeStart:
            self.snoozeDuration = self.snoozeDuration + (datetime.now() - self.pauseStart).seconds
    ...
    def set_status(self, status, resolution_codename=None, user=None):
        ...
        self.save(update_fields=[
            'status', 'previousStatus', 'action',
            'snoozeStart', 'snoozeDuration'
        ])
