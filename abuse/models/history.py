...
class History(CerberusModel):
    ...
    @classmethod
    def log_ticket_action(cls, ticket=None, action=None,
                          user=None, **kwargs):
        """
...
def get_log_message(ticket, action, user, **kwargs):
    ...
    if action in GENERIC_LOG_ACTION:
        ...
        wait = ' (for %d hour(s))' % (ticket.snoozeDuration / 3600) if new_value == 'WaitingAnswer' else ''
        ...
        raise InvalidTicketHistoryAction(
            '{} is not a valid log action'.format(action)
        )
