...
def setup_db():
    ...
    from ..models import (AbusePermission, ServiceAction, Category,
                          MailTemplate, Provider, ReportThreshold,
                          User, Profile, Resolution, Tag, Operator,
                          ApiRoute, Role, BusinessRules)

    Category.create(**{'description': u'Illegal', 'name': u'Illegal', 'label': u'Illegal'})
    Category.create(**{'description': u'Intrusion', 'name': u'Intrusion', 'label': u'Intrusion'})
    Category.create(**{'description': u'Malware', 'name': u'Malware', 'label': u'Malware'})
    Category.create(**{'description': u'Network Attack', 'name': u'Network Attack', 'label': u'Network Attack'})
    Category.create(**{'description': u'Other', 'name': u'Other', 'label': u'Other'})
    Category.create(**{'description': u'Phishing', 'name': u'Phishing', 'label': u'Phishing'})
    spam_category = Category.create(**{'description': u'Spam', 'name': u'Spam', 'label': u'Spam'})
    Category.create(**{'description': u'Copyright', 'name': u'Copyright', 'label': u'Copyright'})

    action = ServiceAction.get_or_create(
        name='default_action',
        module='VPS',
        level='1',
    )

    ReportThreshold.create(
        category=spam_category,
        threshold=100,
        interval=3600,
    )

    for tag in ('distrust:0:autoarchive', 'distrust:1:attach_if_exists', 'distrust:2:no_phishtocheck', 'never_auto_ack'):
    ...
    Provider.create(email='trusted.phishing@provider.com', priority='Critical', apiKey='token')
    Provider.create(email='supertrusted@copyrightprovider.com', priority='High', trusted=True)
