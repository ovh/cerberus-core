...
FALLBACK_FALSE_TEMPLATE = {
    ...
    'regexp': {
        'ips': {
            'pattern': r'(?:abuse\s*report\s*about\s*IP\s*:\s*)' + Parser.ipv4_re,
        },
        'category': {
            'value': 'Spam',
        },
...
class TestParser(CerberusTest):
    ...
    def setUpClass(cls):
        ...
        cls.parser = Parser()
        cls.parser.email_templates[FALLBACK_TRUE_TEMPLATE['email']] = FALLBACK_TRUE_TEMPLATE
        cls.parser.email_templates[FALLBACK_FALSE_TEMPLATE['email']] = FALLBACK_FALSE_TEMPLATE

        for root, dirs, files in os.walk(os.path.abspath(os.path.dirname(__file__)) + '/../samples/'):
            for name in files:
                filename = root + '/' + name
    ...
    def test_get_subject_sample2(self):
        ...
        sample = self.samples['sample2']
        parsed_email = self.parser.parse_from_email(sample)
        self.assertIn('[SpamCop (213.251.151.160) id:6301094139]{SPAM 07.7} =?UTF-8?Q?Impor', parsed_email.subject)
