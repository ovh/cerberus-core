...
class ApiTestCase(CerberusTest):
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            with patch(*self.patch_getnotif):
                for endpoint in self.static_endpoints:
                    response = self.tester.get(endpoint, headers={"X-API-TOKEN": token})
    ...
    def test_globals(self):
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get("/api/dashboard", headers={"X-API-TOKEN": token})
        self.assertEqual(response.status_code, 200)
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get("/api/toolbar", headers={"X-API-TOKEN": token})
        self.assertEqual(response.status_code, 200)
        response = json.loads(response.get_data())
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get(
                "/api/defendants/top20", headers={"X-API-TOKEN": token}
            )
        self.assertEqual(response.status_code, 200)
        response = json.loads(response.get_data())
    ...
    def test_admin(self):
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get("/api/users", headers={"X-API-TOKEN": token})
            self.assertEqual(response.status_code, 200)
            response = json.loads(response.get_data())
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get(
                "/api/categories", headers={"X-API-TOKEN": token}
            )
        self.assertEqual(response.status_code, 200)
        response = json.loads(response.get_data())
    ...
    def test_search(self):
        ...
        params = {
            "paginate": {"currentPage": 1, "resultsPerPage": 10},
            "queryFields": ["id", "publicId", "category"],
    ...
    def test_ticket(self):
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            with patch(*self.patch_enqueue):
                response = self.tester.get(
                    "/api/tickets/1", headers={"X-API-TOKEN": token}
                )
        self.assertEqual(response.status_code, 200)

        response = self.tester.get(
            "api/tickets/1/actions/list", headers={"X-API-TOKEN": token}
        )
        response = json.loads(response.get_data())
        ...
        response = self.tester.get(
            "api/tickets/1/items", headers={"X-API-TOKEN": token}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_admin_threshold(self):
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get(
                "/api/admin/threshold/1337", headers={"X-API-TOKEN": token}
            )
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            with patch(*self.patch_api_cache_delete):
                response = self.tester.post(
                    "/api/admin/threshold",
                    data=json.dumps(
                        {"category": "Copyright", "interval": 15, "threshold": 15}
                    ),
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            response = self.tester.get(
                "/api/admin/threshold", headers={"X-API-TOKEN": token}
            )
        response = json.loads(response.get_data())
        ...
        with patch(*self.patch_api_cache_get), patch(*self.patch_api_cache_set):
            with patch(*self.patch_api_cache_delete):
                response = self.tester.post(
                    "/api/admin/threshold",
                    data=json.dumps(
                        {"category": "Other", "interval": 30, "threshold": 15}
                    ),
                    headers={"content-type": "application/json", "X-API-TOKEN": token},
                )
        self.assertEqual(response.status_code, 200)
