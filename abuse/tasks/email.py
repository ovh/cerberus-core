...
def send_ticket_email(
    ...
    for recipient in recipients:
        EmailService.send_email(
            ticket, recipient, subject, body, category, attachments=attachments
        )
        History.log_ticket_action(
            ticket=ticket, action="send_email", user=user, email=recipient
        )
...
def _get_merged_attachments(ticket, attachments, email_thread):
    ...
    if _attachments:
        try:
            sanitized = _save_and_sanitize_attachments(ticket, _attachments)
            ...
            if ticket.attachments.filter(filename=storage_filename).exists():
                ...
                ticket.attachments.add(
                    AttachedDocument.create(
                        filename=storage_filename, filetype=filetype, name=name
                    )
                )
...
def _fetch_attachments(
    attachments, merged_attachments, email_thread, attach_email_thread=False
):
        ...
        for attach in attachments:
            if attach.get("content"):
                ...
                try:
                    ...
                    _attachments.append(
                        merged_attachments[
                            (_hash, "{}_{}".format(_hash[:4], attach["name"]))
                        ]
                    )
            elif attach.get("filename"):
                _attachments.append(
                    merged_attachments[(attach["filename"], attach["name"])]
                )
