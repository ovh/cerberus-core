...
from ..models import (History, ServiceActionJob, Resolution,
                      Ticket, User, ServiceAction)
...
def apply_action(ticket_id=None, action_id=None, ip_addr=None,
                 user_id=None, close_ticket=True,
                 resolution_id=None, status='Alarm',
                 bypass_status=False, customer_notify=True):
    """
...
def _cancel_by_status(ticket):
    ...
    logger.error(unicode(
        'Ticket %d is %s, Skipping...' % (ticket.id, ticket.status)
    ))
...
def schedule_action(ticket=None, action=None, seconds=5,
                    ip_addr=None, force=False,
                    snooze_duration=None, **kwargs):
    """
    ...
    if (not any((ticket.snoozeStart, ticket.snoozeDuration)) or
            seconds < ticket.get_action_remaining_time()):
        _create_job(ticket, action, ip_addr, snooze_duration, seconds, params)


def _create_job(ticket, action, ip_addr,
                snooze_duration, seconds, params):
