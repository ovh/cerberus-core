...
from .constants import (
    GENERAL_CHECK_PERM_DEFENDANT_LEVEL, GENERAL_SEARCH_MAPPING,
...
from ...models import (AbusePermission, Category, MassContact,
                       MassContactResult, Profile, Report, History,
...
def logout(request):
    ...
    except (CryptoException, KeyError, jwt.DecodeError,
...
def check_perms(**kwargs):
    ...
    try:
        allowed_cats = AbusePermission.filter(
            user=user.id
        ).values_list(
            'category',
            flat=True
...
def get_users_infos(**kwargs):
    ...
    try:
        users = User.objects.filter(where).values(
            'id', 'username', 'email', 'operator'
        )
...
def get_profiles():
    ...
    """
    a = [
        model_to_dict(p, exclude=['actions'])
        for p in Profile.all().order_by('id')
    ]
    return a
            ...
            for key, val in values['filters']['where'].iteritems():
                for field in values['filters']['where'][key]:
                    if field.keys()[0] in GENERAL_SEARCH_MAPPING:
                        ...
                        if Tag.filter(name__in=field['ticketTag'], tagType='Report').count():
...
def _get_toolbar_count(where, user):
    ...
    # Escalated
    resp['escalatedCount'] = Ticket.filter(
        where,
        escalated=True
    ).order_by('id').distinct().count()
    resp['toValidateCount'] = Report.filter(
        status='ToValidate'
    ).count()
...
def dashboard(**kwargs):
    ...
    """
    user = kwargs['user']
    categories = AbusePermission.filter(
        user=user.id
    ).values_list(
        'category',
        flat=True
...
def get_mass_contact(filters=None):
    ...
    response = []
    try:
        campaigns = MassContact.all().order_by(*sort)[(offset - 1) * limit:limit * offset]
...
def _create_jobs(campaign_name, ips, category, body, user):
    ...
    result = MassContactResult.create(
        campaign=campaign,
    )
