...
def get_items_infos(**kwargs):
    ...
    try:
        count = (
            ReportItem.filter(where, report__in=kwargs["reps"])
        ...
        for raw in raw_items:
            items.append(
                ReportItem.filter(where, report__in=kwargs["reps"], rawItem=raw).last()
            )
    except (
        AttributeError,
        KeyError,
    ...
    items = [
        {f.name: f.value_from_object(i) for f in ReportItem._meta.fields} for i in items
    ]
    queue = Queue()
    threads = []
    now = int(time.time())
...
def _check_item_status(item):
    ...
    """
    current_item_infos = {"itemType": item["itemType"], "rawItem": item["rawItem"]}

    current_item_infos.update(
        networking.get_reverses_for_item(
            item["rawItem"], nature=item["itemType"], replace_exception=True
        )
    )
    ...
    try:
        ...
        if resp["report"].ticket:
            History.log_ticket_action(
                ticket=resp["report"].ticket, action="add_item", user=user
            )
    except (
        AttributeError,
        FieldError,
...
def update(item_id, body, user):
    ...
    try:
        ...
        if resp["report"].ticket:
            History.log_ticket_action(
                ticket=resp["report"].ticket, action="update_item", user=user
            )
...
def _get_item_infos(body, user):
    ...
    item_infos.update(
        networking.get_reverses_for_item(
            item_infos["rawItem"], nature=item_infos["itemType"]
        )
    )
...
def delete_from_report(item_id, rep, user):
    ...
    try:
        ...
        if report.ticket:
            History.log_ticket_action(
                ticket=report.ticket, action="delete_item", user=user
            )
...
def update_item_report_and_ticket(item, customer_id, service, user):
    ...
    if item["report"].defendant and customer_id != item["report"].defendant.customerId:
        message = (
            "Resolved customerId for this item (%s) does not "
    ...
    if item["report"].service and service["name"] != item["report"].service.name:
        message = (
            "Resolved service for this item (%s) does not "
        ...
        if all((defendant, service_obj)):
            ...
            if item["report"].ticket and not item["report"].ticket.defendant:
                ...
                item["report"].ticket.reportTicket.all().update(
                    service=service_obj, defendant=defendant
                )
        else:
...
def get_screenshot(item_id, report_id):
    ...
    try:
        ...
        results = {"rawItem": item.rawItem, "screenshots": screenshots}
        return results
    except (PhishingServiceException, InvalidFormatError, SchemaNotFound):
...
def unblock_item(item_id, report_id=None, ticket_id=None):
    ...
    try:
        ...
        if ticket_id:
            ...
            if item.report.id not in ticket.reportTicket.all().values_list(
                "id", flat=True
            ):
    ...
    enqueue("phishing.unblock_url", url=item.rawItem)
