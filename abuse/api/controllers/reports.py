...
from .constants import (IP_CIDR_RE, REPORT_STATUS, REPORT_FILTER_MAPPING,
                        REPORT_FIELDS, REPORT_ATTACHMENT_FIELDS,
                        ALL_CATEGORIES)
from ...models import (AbusePermission, Defendant, Report,
                       ReportItem, Service, Tag, Ticket)
...
def _generate_request_filters(filters, user):
    ...
    allowed = list(AbusePermission.filter(user=user.id).values_list('category', flat=True))
...
def get_items_screenshot(**kwargs):
    ...
    for item in items:
        thread = Thread(target=_get_url_screenshot, args=(item, report.id, queue, only_taken))
        thread.start()
