...
from ...models import (MailTemplate, Ticket, TicketAction,
...
def _get_ordered_presets(user, fields, with_template):
    ...
    presets = []
    preset_groups = TicketWorkflowPreset.filter(
    ...
    for group in preset_groups:
        result = list(TicketWorkflowPreset.filter(
            roles=user.operator.role,
            groupId=group
        ).values(
            *fields
        ...
        if with_template:
            for res in result:
                res['templates'] = list(TicketWorkflowPreset.get(
                    id=res['id'],
                    roles=user.operator.role,
...
def show(user, preset_id):
    ...
    preset['roles'] = list(TicketWorkflowPreset.get(
        id=preset['id']
...
def create(body):
    ...
    try:
        ...
        preset = TicketWorkflowPreset.create(codename=body['codename'], name=body['name'])
