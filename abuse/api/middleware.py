...
UNAUTHENTICATED_ENDPOINTS = ("misc_views.auth", "misc_views.monitor")
...
def setup_middleware(api):
    ...
    def check_token():
        ...
        if (
            ...
            or request.endpoint
            and request.endpoint in UNAUTHENTICATED_ENDPOINTS
            or request.endpoint
            and request.endpoint.startswith("rq_dashboard.")
        ):
            return
    ...
    def _check_allowed_routes():
        ...
        try:
            ...
            is_valid = RoleCache.is_valid(
                role_codename, request.method, request.endpoint
            )
            if is_valid:
                return True, None
            return (
                False,
                "You are not allowed to {} {}".format(request.method, request.path),
            )
