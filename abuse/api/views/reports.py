...
report_views = Blueprint("report_views", __name__, url_prefix="/reports")
...
@perm_required
@validate_body({Optional("domainToRequest"): Any(str, unicode, None)})
...
def get_report_items(report=None):
    ...
    return ReportItemsController.get_items_report(
        rep=report, filters=request.args.get("filters")
    )
...
def get_all_items_screenshot(report=None):
    ...
    return ReportsController.get_items_screenshot(
        report=report, filters=request.args.get("filters")
    )
