...
TEMPLATE = {
    ...
    'regexp': {
        'ips': {
            'pattern': r'(?:Associated\s*IP\s*Address\s*:\s*)' + regexp.IPV4,
        },
        'urls': {
            'pretransform': pretransform,
            'pattern': r'(?:The\s*following\s*URLs\s*are\s*some\s*components\s*of\s*this\s*phishing\s*attack\s*:\s*)' + regexp.URL,
        },
        'category': {
            'pattern': r'((.|\n|\r|\t)*)',
            'transform': True
